...
from Qt.QtCore import QRect, QPoint, QRectF, QSize, QLineF, QPointF, QEventLoop,Signal
from Qt.QtGui import QColor, QPainterPath, QKeySequence, QPixmap, QPen, QBrush, QImage, QPainter, \
    QPolygonF, QClipboard, QCursor, QMouseEvent
from Qt.QtWidgets import QGraphicsView, QApplication, QGraphicsScene, QShortcut, QFileDialog, QDialog, QDesktopWidget
...
class Screenshot(QGraphicsView):
    ...
    def _show(self):
        ...
        self.setGeometry(screenGeometry.x(), screenGeometry.y(), screenGeometry.width(), screenGeometry.height())
    ...
    @staticmethod
    def take_screenshot(flags, parent = None):
        loop = QEventLoop()
        screen_shot = Screenshot(flags, parent)
        screen_shot.show()
    ...
    def getscreenshot(self):
        ...
        screenGeometry = QApplication.primaryScreen().virtualGeometry()
        try:
            self.screenPixel = QApplication.primaryScreen().grabWindow(QApplication.desktop().winId(), screenGeometry.x(), screenGeometry.y(), screenGeometry.width(), screenGeometry.height())
        except:
            self.screenPixel = QApplication.primaryScreen().grabWindow(int(QApplication.desktop().winId()), screenGeometry.x(), screenGeometry.y(), screenGeometry.width(), screenGeometry.height())
    ...
    def mouseMoveEvent(self, event):
        ...
        :return:
        """
        # _screen = QApplication.desktop().screenNumber(QApplication.desktop().cursor().pos())
        # if self._screen != _screen:
    ...
    def redraw(self):
        ...
        if self.action != ACTION_SELECT:
            ...
            if self.penSetBar is not None:
                self.penSetBar.show()
                _pen_point = QPoint(dest.toPoint() + QPoint(0, self.tooBar.height() + spacing))
    ...
    def drawSizeInfo(self):
        ...
        self.itemsToRemove.append(self.graphicsScene.addRect(QRectF(sizeInfoArea), QPen(Qt.white), QBrush(Qt.black)))
